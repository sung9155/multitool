      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        {tools.map((t) => (
          <Link
            key={t.slug}
            to={`/t/${t.slug}`}
            className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-4 transition-colors hover:border-indigo-500/50 hover:bg-zinc-900"
          >
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-zinc-400">{tool.description}</p>
      </div>
      <Body />
    </div>
  );
